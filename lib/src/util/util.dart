      ...
      if (value is List) {
        ...
        mapX.forEach((key, value) {
          if (mapX[key] != mapY[key]) {
            log('mapX[$key] != mapY[$key]. ${mapX[key]} != ${mapY[key]}');
            log('${mapX[key].toString().length} ... ${mapY[key].toString().length}');
          }
        });
        equalityTestResult = mapEquals(value, mapB[key]);
      } else {
        log('value is not list or map for key $key');
